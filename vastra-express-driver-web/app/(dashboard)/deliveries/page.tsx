...
function TaskCard({ task }: { task: DeliveryAssignment }) {
  ...
  return (
    <Link
      href={`/deliveries/${task.id}`}
      className="block bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md hover:border-emerald-200 transition-all group"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-1">
            ...
            {task.order.finalWeight ? ` • ${task.order.finalWeight} kg` : ''}
          </p>
        </div>
        <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-emerald-500 transition-colors flex-shrink-0 mt-1" />
      </div>
    </Link>
  );
}
...
export default function DeliveriesPage() {
  ...
  const activeCount = deliveryTasks.filter((t) =>
      ...
      {isLoading && deliveryTasks.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="font-medium text-gray-700">
            {filter === 'active' ? 'No active deliveries' : 'No tasks found'}
          </p>
